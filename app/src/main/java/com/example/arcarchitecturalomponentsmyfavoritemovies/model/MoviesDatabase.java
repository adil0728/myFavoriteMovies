...
public abstract class MoviesDatabase extends RoomDatabase {
    ...
    private static RoomDatabase.Callback callback = new RoomDatabase.Callback(){
        ...
        public void onCreate(@NonNull SupportSQLiteDatabase db) {
            ...
            new InitialDataAsyncTusk(instance).execute();

        }
    };

    private static class InitialDataAsyncTusk extends AsyncTask<Void, Void, Void>{

        private GenreDao mGenreDao;
        private MovieDao mMovieDao;

        public InitialDataAsyncTusk(MoviesDatabase moviesDatabase) {
           mGenreDao = moviesDatabase.getGenreDao();
           mMovieDao = moviesDatabase.getMovieDao();
        }
        ...
        protected Void doInBackground(Void... voids) {
            ...
            mGenreDao.insert(comedyGenre);
            mGenreDao.insert(romanceGenre);
            mGenreDao.insert(dramaGenre);
            ...
            mMovieDao.insert(movie1);
            mMovieDao.insert(movie2);
            mMovieDao.insert(movie3);
            mMovieDao.insert(movie4);
            mMovieDao.insert(movie5);
            mMovieDao.insert(movie6);
            mMovieDao.insert(movie7);
            mMovieDao.insert(movie8);
            mMovieDao.insert(movie9);
